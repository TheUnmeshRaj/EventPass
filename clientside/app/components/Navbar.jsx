  ...
  return (
    <nav className="bg-slate-900 text-white p-4 flex justify-between items-center sticky top-0 z-50 border-b border-slate-700">
      ...
      <div className="flex gap-2 text-sm">
        <button
          onClick={() => setView("marketplace")}
          className={`px-3 py-1 rounded-full ${view === "marketplace"
            ? "bg-slate-700"
        ...
        <button
          ...
          className={`px-3 py-1 rounded-full ${view === "wallet"
            ? "bg-slate-700"
            : "text-slate-400 hover:text-white"
            }`}
        >
        ...
        <button
          ...
          className={`px-3 py-1 rounded-full flex items-center gap-1 ${view === "balance"
        ...
        <button
          ...
          className={`px-3 py-1 rounded-full ${view === "dashboard"
            ? "bg-slate-700"
            : "text-slate-400 hover:text-white"
            }`}
        >
            ...
            <div className="relative">
              ...
              {accountOpen && (
                <div className="absolute right-0 mt-2 w-48 bg-slate-800 text-white rounded shadow-lg p-2 z-50">
                  <button
                    onClick={() => { setView("user-profile"); setAccountOpen(false); }}
                    className="w-full text-left px-2 py-2 hover:bg-slate-700 rounded text-sm"
                  >
                    My Profile
                  </button>
                  ...
                  {authUser?.email === "unmeshraj.raj@gmail.com" && (
                    <button
                      onClick={() => { setView("admin-events"); setAccountOpen(false); }}
                      className="w-full text-left px-2 py-2 hover:bg-slate-700 rounded text-sm"
                    >
                      Manage Events
                    </button>
