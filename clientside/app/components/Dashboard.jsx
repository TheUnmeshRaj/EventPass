...
export default function Dashboard({ ledger }) {
  return (
          ...
          {ledger.map((block, idx) => (
            <div key={idx} className="flex p-4 text-slate-300 hover:bg-slate-800/50 transition-colors">
              ...
              <div className="flex-1">
                {block.type === 'MINT' && (
                  <span>
                    Minted <span className="text-white">{block.details.ticketId}</span> for{' '}
                    <span className="text-emerald-400">₹{block.details.price}</span> to {block.details.buyer}
                  </span>
                )}
                {block.type === 'BURN' && (
                  <span>
                    Ticket <span className="text-white">{block.details.ticketId}</span> returned by{' '}
                    {block.details.prevOwner}
                  </span>
                )}
