...
export function Ledger({ ledger = [], loading = false, userLookup = {} }) {
  ...
  return (
    <div className="px-4 py-6 sm:px-6 max-w-6xl mx-auto w-full">
      ...
      <div className="bg-slate-900 rounded-xl overflow-hidden shadow-xl border border-slate-700 font-mono text-sm hidden lg:block">
        ...
        <div className="max-h-[500px] overflow-y-auto divide-y divide-slate-800">
          {ledger.map((block, idx) => (
            <div key={block.id || block.hash || idx} className="flex p-4 text-slate-300 hover:bg-slate-800/50 transition-colors">
              <div className="w-32">{renderTypeBadge(block.type)}</div>
      ...
      <div className="lg:hidden space-y-3">
        {ledger.map((block, idx) => (
          <div
            key={block.id || block.hash || idx}
            className="bg-slate-900 rounded-2xl border border-slate-800 p-4 text-slate-200 shadow-lg"
          >
            <div className="flex items-center justify-between gap-3 text-xs mb-3">
              {renderTypeBadge(block.type)}
              <span className="text-slate-500">{renderTimestamp(block.timestamp)}</span>
            </div>
            <p className="text-sm text-slate-300 leading-relaxed">{renderDetails(block)}</p>
            <p className="mt-3 text-[11px] text-slate-500 break-all">{block.hash}</p>
          </div>
        ))}
      </div>
    </div>
